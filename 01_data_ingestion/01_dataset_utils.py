...
def create_streaming_simulation_data(df, output_path, batch_size=10, num_batches=5, seed=42):
    ...
    for i in range(num_batches):
        ...
        batch_path = f"{output_path}_batch_{i+1}.json"
        
        # Save batch as JSON
        batch_df.to_json(batch_path, orient='records', lines=True)
        
        batch_files.append(batch_path)
        
    print(f"Created {num_batches} streaming data batches")
...
def analyze_dataset_characteristics(df):
    """
    Analyze dataset characteristics to identify potential issues.
