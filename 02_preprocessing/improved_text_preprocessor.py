...
def preprocess_text(df, cache=True):
    ...
    - Improved acronym handling in any context
    ...
    if columns_to_preprocess:
        ...
        # Using word boundary markers (\b) to ensure we only match whole acronyms
        # The regex patterns now match acronyms in any context (beginning, middle, or end of text)
        acronyms = {
            r"\bU\.S\.A\.\b": "USA ",  # Must come before U.S. to avoid partial replacement
            r"\bU\.S\.\b": "US ",      # Match U.S. as a standalone acronym
            r"\bU\.N\.\b": "UN ",
            r"\bF\.B\.I\.\b": "FBI ",
            r"\bC\.I\.A\.\b": "CIA ",
            r"\bD\.C\.\b": "DC ",
        ...
        for col_name in columns_to_preprocess:
            # First apply acronym normalization (must happen before lowercase)
            # This is done separately to ensure proper handling of capitalized acronyms
