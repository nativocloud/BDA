def analyze_dataset_characteristics(df, save_path=None):
    ...
    """
    from pyspark.sql.functions import col, length, count, when, isnan, isnull, avg, min as spark_min, max as spark_max, countDistinct
    ...
    if "label" in columns:
        ...
        if len(class_counts) > 1:
            # Use Python's built-in min/max functions explicitly to avoid conflicts with PySpark
            min_count = builtins.min(class_counts.values())
            max_count = builtins.max(class_counts.values())
    ...
    for column_name in columns:
        ...
        null_data.append({"column": column_name, "null_count": null_count, "null_percentage": null_pct})
    ...
    if "label" in columns and len(class_counts) > 1 and class_balance is not None:
    ...
    results = {
        ...
        "text_length_stats": {
            "avg": length_stats.collect()[0]["avg_length"] if length_stats is not None else None,
            "min": length_stats.collect()[0]["min_length"] if length_stats is not None else None,
            "max": length_stats.collect()[0]["max_length"] if length_stats is not None else None,
            "quantiles": {
                "25%": quantiles[0] if quantiles is not None else None,
                "50%": quantiles[1] if quantiles is not None else None,
                "75%": quantiles[2] if quantiles is not None else None
            }
        } if "text" in columns and length_stats is not None else None,
        "short_texts": {
            "count": short_texts,
